...
function validateCouponScope(coupon, items) {
  ...
  const matchingItems = items.filter((item) => {
    ...
    if (coupon.excludedProductIds && coupon.excludedProductIds.length > 0) {
      const isExcluded = coupon.excludedProductIds.some((exId) => exId.toString() === productId);
    ...
    if (coupon.applicableTo === "products") {
      return coupon.productIds && coupon.productIds.some((pid) => pid.toString() === productId);
    }
    ...
    if (coupon.applicableTo === "services") {
      return coupon.serviceIds && coupon.serviceIds.some((sid) => sid.toString() === productId);
    }
